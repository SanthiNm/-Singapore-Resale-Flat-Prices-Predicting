...
# Load the model
with open("B:\Project\.venv\house_price_model.pkl", 'rb') as f:
    model = pickle.load(f)
...
st.markdown(
    """
    <h1 style='color: orange;'>House Price Prediction App</h1>
    """,
...
for feature in features:
    if feature in categorical_mappings:
        selected_option = st.selectbox(f"Select {feature.replace('_', ' ').capitalize()}:", options=list(categorical_mappings[feature].keys()))
        input_data[feature] = categorical_mappings[feature][selected_option]
    else:
        if feature in ['floor_area_sqm']:  # Example of a feature that should remain a float
            input_data[feature] = st.number_input(f"{feature.replace('_', ' ').capitalize()}:", min_value=0.0, step=0.1)
        else:
            input_data[feature] = st.number_input(f"{feature.replace('_', ' ').capitalize()}:", min_value=0, step=1, format="%d")
